    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start mb-10 gap-6">
          <div>
            ...
            <p className="text-secondary-text">Engineering report generated for <span className="text-primary-text font-semibold">{summary.project_name}</span></p>
          </div>
          <button
            onClick={onReset}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-background/40 border border-border/40 rounded-2xl p-6 text-center">
             <p className="text-xs font-bold text-secondary-text/60 uppercase tracking-tighter mb-2">Overall Health</p>
             <div className={`text-4xl font-black ${aiReport ? getScoreColor(aiReport.overall_score) : 'text-accent'}`}>
               {aiReport ? aiReport.overall_score : '--'}<span className="text-lg font-normal opacity-40">/100</span>
             </div>
          </div>
          <div className="bg-background/40 border border-border/40 rounded-2xl p-6 text-center">
          ...
          <div className="bg-background/40 border border-border/40 rounded-2xl p-6 text-center">
            <p className="text-xs font-bold text-secondary-text/60 uppercase tracking-tighter mb-2">Supported</p>
            <p className="text-3xl font-bold text-accent">{summary.total_supported_files}</p>
          </div>
        </div>
      </motion.div>
      ...
      {aiReport && (
        <>
          ...
          <motion.div
             initial={{ opacity: 0, y: 20 }}
             animate={{ opacity: 1, y: 0 }}
             className="bg-accent/5 border border-accent/10 rounded-[2rem] p-8"
          >
             <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                   ...
                   <p className="text-xs text-secondary-text">Guidance for project evolution</p>
                </div>
             </div>
      ...
      {!aiReport && (
        <div className="bg-yellow-500/5 border border-yellow-500/10 rounded-2xl p-6 text-center">
           <p className="text-sm text-yellow-500 font-bold">AI Analysis was not performed (Missing API Key or failed analysis).</p>
           <p className="text-xs text-secondary-text mt-1">Please configure GEMINI_API_KEY for a full engineering report.</p>
        </div>
      )}
    </div>
  );
